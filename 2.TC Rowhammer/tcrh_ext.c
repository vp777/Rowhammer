...
#define T_RADIUS 6 //How far away from the current "row" to hammer, set it to -1 to check every row
#define B_RADIUS 2 //How far away from the current "row" to search for SBSR, set it to -1 to check every row
...
//	const uintptr_t taddr[2] = { oaddr, oaddr+64 };
	...
	for (size_t i=offset; i<MIN(offset+(uint32_t)B_RADIUS, len); i++) {
...
int main(int argc, char *argv[]) {
	...
	for (size_t i = 0; i < n; i++) {
		...
		for (int j = i + 1; j < MIN(i+1+(uint32_t)T_RADIUS, n); j++) {
